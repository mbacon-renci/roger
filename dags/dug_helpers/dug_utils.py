...
class Dug:
    ...
    def annotate_files(self, parser_name, parsable_files):
        ...
        output_base_path = path.dug_annotation_path('')
        ...
        for parse_file in parsable_files:
            ...
            path.write_object({}, os.path.join(elements_file_path, 'element_file.json'))
            ...
            path.write_object(elements, elements_out_file)
            ...
            path.write_object(non_expanded_concepts, concepts_out_file)
    ...
    def validate_indexed_elements(self, elements_file):
        elements = [x for x in path.read_object(elements_file) if not isinstance(x, DugConcept)]
        # Pick ~ 10 %
    ...
    def crawl_concepts(self, concepts, data_set_name):
        ...
        :return:
        """
        crawl_dir = path.dug_crawl_path('crawl_output')
        ...
        output_file = path.dug_expanded_concepts_path(output_file_name)
        extracted_output_file = path.dug_expanded_concepts_path(extracted_dug_elements_file_name)
        ...
        for concept_id, concept in concepts.items():
            ...
            if percent_complete % 10 == 0:
                log.info(f"{percent_complete}%")
        path.write_object(obj=concepts, path=output_file)
        path.write_object(obj=extracted_dug_elements, path=extracted_output_file)
...
class DugUtil():
    ...
    def clear_annotation_cached(config=None, to_string=False):
        with Dug(config, to_string=to_string) as dug:
            annotation_path = path.dug_annotation_path("")
            path.clear_dir(annotation_path)
    ...
    def make_kg_tagged(config=None, to_string=False):
        with Dug(config, to_string=to_string) as dug:
            output_base_path = path.dug_kgx_path("")
            path.clear_dir(output_base_path)
            log.info("Starting building KGX files")
            elements_files = path.dug_elements_objects()
            for file in elements_files:
                elements = path.read_object(file)
                if "topmed_" in file:
                ...
                path.write_object(kg, output_file_path)
    ...
    def index_variables(config=None, to_string=False):
        with Dug(config, to_string=to_string) as dug:
            ...
            elements_object_files = path.dug_elements_objects()
    ...
    def index_extracted_elements(config=None, to_string=False):
        with Dug(config, to_string=to_string) as dug:
            elements_object_files = path.dug_extracted_elements_objects()
            for file in elements_object_files:
                dug.index_elements(file)
            output_log = dug.log_stream.getvalue() if to_string else ''
        return output_log
    ...
    def index_concepts(config=None, to_string=False):
        with Dug(config=config, to_string=to_string) as dug:
            ...
            dug.clear_kg_index()
            expanded_concepts_files = path.dug_expanded_concept_objects()
            for file in expanded_concepts_files:
                concepts = path.read_object(file)
    ...
    def validate_indexed_concepts(config=None, to_string=False):
        with Dug(config, to_string=to_string) as dug:
            ...
            expanded_concepts_files_dict = {
                get_data_set_name(file): file for file in path.dug_expanded_concept_objects()
            }
            annotated_elements_files_dict = {
                get_data_set_name(file): file for file in path.dug_elements_objects()
            }
            try:
            ...
            for data_set_name in annotated_elements_files_dict:
                ...
                dug_elements = path.read_object(elements_file_path)
                dug_concepts = path.read_object(concepts_file_path)
...
def get_versioned_files(config: RogerConfig, data_format, output_file_path, data_store="s3", unzip=False):
    ...
    """
    meta_data = path.read_relative_object("../metadata.yaml")
    output_dir: Path = path.dug_input_files_path(output_file_path)
    # clear dir
    path.clear_dir(output_dir)
